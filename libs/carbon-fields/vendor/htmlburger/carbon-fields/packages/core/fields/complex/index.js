...
import { get, find, isEmpty, isString, template } from "lodash";
...
class ComplexField extends Component {
  ...
  get inserterButtonText() {
    ...
    return sprintf(
      __("Add %s", "carbon-fields-ui"),
      field.labels.singular_name
    );
  }
  ...
  getAvailableGroups(key) {
    ...
    return field.groups.filter(
      ({ name }) => existingGroupNames.indexOf(name) === -1
    );
  }
  ...
  handleRemoveGroup = (groupId) => {
    const { value, groupIdKey, onRemoveGroup } = this.props;
  ...
  render() {
    ...
    return (
      <Fragment>
        ...
        {!value.length && (
          <ComplexPlaceholder
            label={__("Es gibt noch keine Einträge.", "carbon-fields-ui")}
          >
            <ComplexInserter
              ...
              {value.map((group, index) => (
                ...
                <ComplexGroup
                  ...
                  {...onGroupSetup(group, {
                    ...
                    allowClone:
                      field.duplicate_groups_allowed && !this.isMaximumReached,
        ...
        {!this.isTabbed && !!value.length && (
          <div className="cf-complex__actions">
            ...
            >
              {allGroupsAreCollapsed
                ? __("Expand All", "carbon-fields-ui")
                : __("Collapse All", "carbon-fields-ui")}
            </button>
          </div>
        )}
      </Fragment>
...
addFilter(
  ...
  (OriginalField) => (props) => {
    ...
    return (
      <OriginalField className={`cf-complex--${field.layout}`} {...props} />
    );
  }
);

addFilter(
  "carbon-fields.complex.validate",
  "carbon-fields/core",
  (field, value) => {
    const { min, labels, required } = field;
    ...
    if (min > 0 && value.length < min) {
      ...
      return sprintf(
        __(
          "Minimum number of rows not reached (%1$d %2$s)",
          "carbon-fields-ui"
        ),
        Number(min),
        label.toLowerCase()
